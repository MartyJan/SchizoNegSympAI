...
Usage:
    python estimator.py path/to/text_file.txt --model gpt-4
    python estimator.py path/to/folder --model gemini-pro --output /path/to/output
"""
...
import tiktoken
from constants import ITEM_ABBR_TO_NAME, ITEM_TYPES_TO_ABBR_LIST
...
def run_single(input_path: Path, output_dir: Path, model_id: str) -> None:
    ...
    output_path = output_dir / f"{filename}_{now.strftime('%Y-%m-%d-%H-%M-%S')}.json"
    with output_path.open("w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False)
    print(f"[{filename}] Output saved to {output_path}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("input", type=str, help="File or folder of input text")
    parser.add_argument(
        "--model",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default=config["llm"]["path"]["result_dir"],
