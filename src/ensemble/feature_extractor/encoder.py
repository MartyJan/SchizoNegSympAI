...
class FeatureExtractor:
    ...
    def get_facial_feature(self, df: pd.DataFrame) -> np.ndarray:
        ...
        assert (
            BSHAPE_END_IDX - BSHAPE_START_IDX + 1
        ) == 52, "Blendshape columns are incorrect"
        ...
        while (end := start + self.n_video_frames) < len(df):
            ...
            min_ratio = self.config["ensemble"]["encoder"]["min_valid_video_frame_ratio"]
            
            if len(wnd_high_conf) / len(wnd) >= min_ratio:
                blendshapes = wnd_high_conf.iloc[:, BSHAPE_START_IDX:BSHAPE_END_IDX + 1]
    ...
    def get_pose_feature(self, df: pd.DataFrame) -> np.ndarray:
        ...
        assert (
            POSE_X_END_IDX - POSE_X_START_IDX
        ) == 24 * 4, "Pose columns are incorrect"
        ...
        while (end := start + self.n_video_frames) < len(df):
            ...
            if len(wnd_high_conf) / len(wnd) >= min_ratio:
                ...
                y = wnd_high_conf.iloc[:, (POSE_X_START_IDX + 1):(POSE_X_END_IDX + 2):4]
                z = wnd_high_conf.iloc[:, (POSE_X_START_IDX + 2):(POSE_X_END_IDX + 3):4]
    ...
    def get_audio_feature(self, waveform: Tensor) -> np.ndarray:
        """
        ...
        return self.normalize_np(np.stack(audio_wnd_arr)) if audio_wnd_arr else np.array([])
    ...
    def preprocess(self, landmark_file_path: Path, audio_file_path: Path) -> Optional[Tuple[pd.DataFrame, Tensor, Tensor]]:
        """
        Preprocess audio and landmark data.
        
        Loads and processes landmark data from CSV file and audio data from WAV file,
        ...
        Returns:
            Optional[Tuple[pd.DataFrame, Tensor, Tensor]]: 
                - Preprocessed landmark DataFrame with silent frames removed
                - Normalized non-silent audio tensor
                - Full audio tensor resampled to target rate
                - Returns None if any preprocessing steps fail
        """
        # Load and preprocess audio waveform
        try:
            audio, audio_sample_rate = torchaudio.load(audio_file_path)  # (channel, timestamp)
        ...
        try:
            ...
            print(f"Error loading landmarks {landmark_file_path}: {e}")
            return None
        ...
        audio = torchaudio.functional.resample(
            audio, audio_sample_rate, self.audio_fps
        )
        normalized_audio = audio / (torch.max(torch.abs(audio)) + 1e-8)  # Avoid division by zero
...
def process_patient_data(
    ...
    audio_file_path = audio_dir / patient / "audio" / f"{patient}_{diarization_map[patient]}.wav"

    # Check if files exist
    if not landmark_file_path.exists():
        print(f"Landmark file not found for {patient}. Skipping.")
        return
        
    if not audio_file_path.exists():
        print(f"Audio file not found for {patient}. Skipping.")
        return

    # Preprocess audio and landmark data
    preprocessed_data = extractor.preprocess(landmark_file_path, audio_file_path)
    if preprocessed_data is None:
        print(f"Preprocessing failed for {patient}. Skipping.")
        return
        
    landmark_df, nonsilent_audio, audio = preprocessed_data

    # Extract features
    features_dict["face"] = extractor.get_facial_feature(landmark_df)
    features_dict["pose"] = extractor.get_pose_feature(landmark_df)
    features_dict["audio"] = extractor.get_audio_feature(nonsilent_audio.to(extractor.device))
    ...
    for modality, feature_np in features_dict.items():
        if feature_np.size == 0:
            print(f"Warning: Empty feature array for {patient}'s {modality}. Skipping.")
            continue
            
        embed_dir = Path(config["ensemble"]["path"]["embed_dir"]) / patient / modality
...
def encode() -> None:
    ...
    if not diarization_file.exists():
        print(f"Diarization file not found: {diarization_file}")
        return
        
    try:
        diarization_map = get_diarization_map(diarization_file)
    except Exception as e:
        print(f"Error loading diarization map: {e}")
        return
    ...
    print(f"Processing complete. Processed {patient_count} patients.")
