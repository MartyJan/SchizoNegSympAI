...
def holistic_tracking_on_video(
    ...
    with mp.solutions.holistic.Holistic(
        ...
        while cap.isOpened():
            ...
            timestamp += 1
            print(f"Processing video: {vid_file.name} | Frame: {timestamp}")
...
def visualize_face_result(
    ...
    annotated_face_image = draw_face_landmarks_on_image(
        face_image, face_result
    )
    cv2.imwrite(
    ...
    blendshapes_bar = plot_face_blendshapes_bar_graph(
...
def draw_face_landmarks_on_image(rgb_image: np.ndarray, detection_result: vision.FaceLandmarkerResult) -> np.ndarray:
    """
...
def run_single(
    ...
    if not is_video_file(input_path):
        print(f"Error: {input_path} is not a valid video file")
        return
